...
import com.binfile.opener.viewer.reader.pdf.converter.utils.UtilsCodeAndroid;
import com.binfile.opener.viewer.reader.pdf.converter.utils.CodeAreaPainter;
import com.binfile.opener.viewer.reader.pdf.converter.utils.TextStyles;
...
public class PaintersDefaultArea implements CodeAreaPainter, SimpleColorCapableAreaPainters {

    
    protected final AreaCodeDefaultCodeAndroidAreasCodeArea areaCode;
    ...
    private final CodeAreaScrollPanesDefault scrollPanel;
    
    private final CodeAreaMouseListenerDefault codeAreaMouseListener;
    ...
    private final SimpleCodeAreasMetric metrics = new SimpleCodeAreasMetric();
    ...
    private final SimpleCodeAreasDimension dimensions = new SimpleCodeAreasDimension();
    
    private final SimpleCodeVisibility visibility = new SimpleCodeVisibility();

    
    private final BasicCodeAreaLayout layout = new BasicCodeAreaLayout();
    
    private SimpleCodeAreasColor colorsProfile = new SimpleCodeAreasColor();
    ...
    public PaintersDefaultArea(final AreaCodeDefaultCodeAndroidAreasCodeArea areaCode) {
        this.areaCode = areaCode;

        scrollPanel = new CodeAreaScrollPanesDefault(areaCode.getContext()) {
            @Override
            protected void onScrollChanged(int horizontal, int vertical, int oldHorizontal, int oldVertical) {
                ...
                ((ScrollingCapable) areaCode).setScrollPosition(scrolling.getScrollPosition());
            }
        };
        scrollPanel.setLongClickable(true);

        dataView = new View(PaintersDefaultArea.this.areaCode.getContext()) {
            @Override
        ...
        codeAreaMouseListener = new CodeAreaMouseListenerDefault(areaCode, scrollPanel);
    ...
    public void attach() {
        ...
        areaCode.addView(scrollPanel, wrapLayout);
        dataView.setOnTouchListener(codeAreaMouseListener);
        areaCode.addDataChangedListener(codeAreaDataChangeListener);
    }

    @Override
    public void detach() {
        this.areaCode.removeView(scrollPanel);
        dataView.setOnTouchListener(null);
        areaCode.removeDataChangedListener(codeAreaDataChangeListener);
    }
    ...
    public void recomputeLayout() {
        ...
        structure.updateCache(areaCode, charactersPerPage);
        codeCharactersCase = ((CodeCharactersCaseCapable) areaCode).getCodeCharactersCase();
        backgroundPaintMode = ((CapableBackgroundPaint) areaCode).getBackgroundPaintMode();
        showMirrorCursor = ((CaretCapable) areaCode).isShowMirrorCursor();
//        antialiasingMode = ((AntialiasingCapable) codeArea).getAntialiasingMode();
        minRowPositionLength = ((RowWrappingCapable) areaCode).getMinRowPositionLength();
        maxRowPositionLength = ((RowWrappingCapable) areaCode).getMaxRowPositionLength();
    ...
    private void updateCaret() {
        editOperation = ((EditModeCapable) areaCode).getActiveOperation();

        caretChanged = false;
    }

    private void validateCaret() {
        CodeAreaCaret caret = ((CaretCapable) areaCode).getCaret();
        CodeAreaCaretPosition caretPosition = caret.getCaretPosition();
        if (caretPosition.getDataPosition() > areaCode.getDataSize()) {
            caret.setCaretPosition(null);
        }
    }

    private void validateSelection() {
        CodeAreaSelection selectionHandler = ((SelectionCapable) areaCode).getSelectionHandler();
        if (!selectionHandler.isEmpty()) {
            long dataSize = areaCode.getDataSize();
            if (dataSize == 0) {
                ((SelectionCapable) areaCode).clearSelection();
            } else {
                ...
                if (selectionChanged) {
                    ((SelectionCapable) areaCode).setSelection(start, end);
                }
            }
        }
    }
    ...
    private void recomputeDimensions() {
        ...
        int componentWidth = areaCode.getWidth();
        int componentHeight = areaCode.getHeight();
    ...
    public void fontChanged() {
        ...
        charset = ((CharsetCapable) areaCode).getCharset();
        textStyles = ((CapableTextStyle) areaCode).getCodeTextStyles();
        paint = new Paint();
    ...
    private void recomputeScrollState() {
        scrolling.setScrollPosition(((ScrollingCapable) areaCode).getScrollPosition());
        int characterWidth = metrics.getCharacterWidth();

        if (characterWidth > 0) {
            scrolling.updateCache(areaCode, getHorizontalScrollBarSize(), getVerticalScrollBarSize());
...
//        paintMainArea(g);
    ...
    public void paintHeader(Canvas g) {
        ...
        g.clipRect(clipBounds != null ? UtilsCodeAndroid.computeIntersection(headerArea, clipBounds) : headerArea);
...
//        int componentWidth = dimensions.getComponentWidth();
        ...
        if (viewMode == CodeAreaViewMode.DUAL || viewMode == CodeAreaViewMode.CODE_MATRIX) {
            ...
            for (int characterOnRow = skipToChar; characterOnRow < codeCharEnd; characterOnRow++) {
                ...
                if (!UtilsCodeAndroid.areSameColors(color, renderColor)) { // || !colorType.equals(renderColorType)
                    sequenceBreak = true;
                }
                if (sequenceBreak) {
                    ...
                    if (!UtilsCodeAndroid.areSameColors(color, renderColor)) {
                        renderColor = color;
    ...
    public void paintRowPosition(Canvas g) {
        ...
        g.clipRect(clipBounds != null ? UtilsCodeAndroid.computeIntersection(rowPositionsArea, clipBounds) : rowPositionsArea);
...
//    }
    ...
    private void prepareRowData(long dataPosition) {
        ...
        if (dataPosition < dataSize) {
            ...
            BinaryData data = areaCode.getContentData();
            if (data == null) {
    ...
    public void paintRowText(Canvas g, long rowDataPosition, int rowPositionX, int rowPositionY) {
        ...
        for (int charOnRow = skipToChar; charOnRow < skipRestFromChar; charOnRow++) {
            ...
            if (sequenceBreak) {
                if (!UtilsCodeAndroid.areSameColors(lastColor, renderColor)) {
...
//        }
        ...
        CodeAreaCaretDefault caret = (CodeAreaCaretDefault) ((CaretCapable) areaCode).getCaret();
        ...
        Rect intersection = UtilsCodeAndroid.computeIntersection(mainAreaRect, scrolledCursorRect);
        ...
        if (cursorVisible) {
            ...
            CodeAreaCaretDefault.CursorRenderingMode renderingMode = caret.getRenderingMode();
...
//        }
    ...
    private void paintCursorRect(Canvas g, Rect intersection, Rect scrolledCursorRect, Rect cursorRect, CodeAreaCaretDefault.CursorRenderingMode renderingMode, CodeAreaCaretDefault caret) {
...
//                    g.setPaintMode();
            ...
            case NEGATIVE: {
                ...
                BinaryData codeAreaData = areaCode.getContentData();
...
//                        }
    ...
    @Override
    public SimpleCodeAreasColor getBasicColors() {
        return colorsProfile;
    }

    @Override
    public void setBasicColors(SimpleCodeAreasColor colors) {
    ...
    public Rect getPositionRect(long dataPosition, int codeOffset, CodeAreaSection section) {
        ...
        CodeAreaCaretDefault.CursorShape cursorShape = editOperation == EditOperation.INSERT ? CodeAreaCaretDefault.CursorShape.INSERT : CodeAreaCaretDefault.CursorShape.OVERWRITE;
        int cursorThickness = CodeAreaCaretDefault.getCursorThickness(cursorShape, characterWidth, rowHeight);
...
//        dataView.layout(scrollPanelRect.left, scrollPanelRect.top, scrollPanelRect.right, scrollPanelRect.bottom);
        ...
        if (rowHeight > 0 && characterWidth > 0) {
            scrolling.updateCache(areaCode, getHorizontalScrollBarSize(), getVerticalScrollBarSize());
            ScrollViewDimension viewDimension = scrolling.computeViewDimension(areaCode.getWidth(), areaCode.getHeight(), layout, structure, characterWidth, rowHeight);
        ...
        Activity activity = (Activity) areaCode.getContext();
